...
import { getDatabase, getBlocksFromPage } from '../utils/notion';

export const getStaticProps = async () => {
	const cv = await getDatabase(process.env.NOTION_CV_DB);
	let cleanCvData = await Promise.all(
		cv.map(async (cvItem) => {
			let blocks = await getBlocksFromPage(cvItem.id);
			return { item: cvItem, blocks: blocks };
		})
	);
...
export default function Home({ cleanCvData }) {
	const locale = 'fr';
	...
	return (
		<>
			...
			<Curriculum cleanCvData={cleanCvData} />
...
const GetInTouch = styled.div`
	...
	&:before {
		...
		color: ${(props) =>
			props.theme === 'dark' ? Colors.light : Colors.light};
		font-weight: 800;
		font-size: 1.2em;
